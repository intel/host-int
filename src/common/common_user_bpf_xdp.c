...
int xdp_link_attach(int ifindex, __u32 xdp_flags, int prog_fd)
{
    ...
    if (err < 0) {
        EPRT("ifindex(%d) link set xdp fd failed (%d): %s\n", ifindex, -err,
             strerror(-err));

        switch (-err) {
        case EBUSY:
        case EEXIST:
            EPRT("Hint: XDP already loaded on device"
                 " use --force to swap/replace\n");
            break;
        case EOPNOTSUPP:
            EPRT("Hint: Native-XDP not supported"
...
int xdp_link_detach(int ifindex, __u32 xdp_flags, __u32 expected_prog_id)
{
    ...
    if (err) {
        EPRT("Get link xdp id failed (err=%d): %s\n", -err, strerror(-err));
        return EXIT_FAIL_XDP;
    }

    if (!curr_prog_id) {
        VPRT("INFO: %s() no curr XDP prog on ifindex:%d\n", __func__, ifindex);
        return EXIT_OK;
    }

    if (expected_prog_id && curr_prog_id != expected_prog_id) {
        EPRT("%s() expected prog ID(%d) no match(%d), not removing\n", __func__,
             expected_prog_id, curr_prog_id);
        return EXIT_FAIL;
    }

    if ((err = bpf_set_link_xdp_fd(ifindex, -1, xdp_flags)) < 0) {
        EPRT("%s() link set xdp failed (err=%d): %s\n", __func__, err,
             strerror(-err));
        return EXIT_FAIL_XDP;
    }

    VPRT("INFO: %s() removed XDP prog ID:%d on ifindex:%d\n", __func__,
...
struct bpf_object *load_bpf_object_file(const char *filename, int ifindex)
{
    ...
    if (err) {
        EPRT("loading BPF-OBJ file(%s) (%d): %s\n", filename, err,
             strerror(-err));
        return NULL;
    }
...
static struct bpf_object *open_bpf_object(const char *file, int ifindex)
{
    ...
    // obj can be a valid pointer, NULL or err number
    obj = bpf_object__open_xattr(&open_attr);
    if (!obj) {
        EPRT("Couldn't open BPF-OBJ file(%s)\n", file);
        return NULL;
    } else if ((unsigned long)obj >= (unsigned long)MAX_ERRNO) {
        err = (unsigned long)obj;
        EPRT("opening BPF-OBJ file(%s) (%d): %s\n", file, err, strerror(-err));
        return NULL;
    }
    ...
    if (!first_prog) {
        EPRT("file %s contains no programs\n", file);
        return NULL;
    }
                                                   ...
                                                   const char *pin_dir)
{
    ...
    if (!obj) {
        EPRT("failed to open object %s\n", file);
        return NULL;
    }

    err = reuse_maps(obj, pin_dir);
    if (err) {
        EPRT("failed to reuse maps for object %s, pin_dir=%s\n", file, pin_dir);
        return NULL;
    }
...
struct bpf_object *load_bpf_and_xdp_attach(struct config *cfg)
{
    ...
    if (!bpf_prog) {
        EPRT("couldn't find a program in ELF section '%s'\n", cfg->progsec);
        exit(EXIT_FAIL_BPF);
    }
                      ...
                      const struct bpf_map_info *exp)
{
    ...
    if (exp->value_size && exp->value_size != info->value_size) {
        EPRT("%s() Map value size(%d) mismatch expected size(%d)\n", __func__,
             info->value_size, exp->value_size);
        return EXIT_FAIL;
    }
    if (exp->max_entries && exp->max_entries != info->max_entries) {
        EPRT("%s() Map max_entries(%d) mismatch expected size(%d)\n", __func__,
             info->max_entries, exp->max_entries);
        return EXIT_FAIL;
    }
    if (exp->type && exp->type != info->type) {
        EPRT("%s() Map type(%d) mismatch expected type(%d)\n", __func__,
             info->type, exp->type);
        return EXIT_FAIL;
    }
                       ...
                       struct bpf_map_info *info, bool silent)
{
    ...
    if (len < 0) {
        EPRT("constructing full mapname path\n");
        return -1;
    }
    ...
    if (fd < 0) {
        if (!silent) {
            EPRT("Failed to open bpf map file:%s err(%d): %s\n", filename,
